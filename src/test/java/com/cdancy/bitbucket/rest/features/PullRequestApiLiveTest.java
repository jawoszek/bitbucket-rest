...
import com.cdancy.bitbucket.rest.*;
...
public class PullRequestApiLiveTest extends BaseBitbucketApiLiveTest {
    ...
    private static final String testUserName = "testUserName2";
    private static final String testUserPassword = "testUserPassword2";
    ...
    @Test (dependsOnMethods = "testPutExistingParticipant")
    ...
    public void testAssignParticipants() {
        ...
        for (final User possibleUser : userPage.values()) {
            System.out.println("---------------------------------------");
            System.out.println(possibleUser.name());
            System.out.println("---------------------------------------");
    ...
    @Test (dependsOnMethods = "testAssignParticipants")
    public void testPutNonExistentParticipant() {
        ...
        final Participants localParticipants =
            testUserApi.pullRequestApi().putParticipant(project, repo, prId, testUser.slug(), createParticipants);
    ...
    @Test (dependsOnMethods = "testPutNonExistentParticipant")
    public void testPutExistingParticipant() {
        User testUser = getTestUser();
    ...
    private BitbucketApi apiForTestUser() {
        final BitbucketAuthentication creds = BitbucketAuthentication
            .builder()
            .credentials(testUserName + ":" + testUserPassword)
            .build();
    ...
    private User getTestUser() {
        final UserPage userPage = api.adminApi().listUsers(testUserName, null, null);
        assertThat(userPage.values().isEmpty()).isFalse();
        return userPage.values().get(0);
    }

    private void addTestUser() {
        api.adminApi().postUser(testUserName, testUserPassword, testUserName, "test@test.test", true, null);
    }

    private void deleteTestUser() {
        api.adminApi().deleteUser(testUserName);
    }
}
